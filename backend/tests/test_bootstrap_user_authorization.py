...
import app.database.models  # Register all SQLAlchemy models with Base
...
from app.identity.repositories import role_repository, user_role_repository, identity_user_repository


def create_test_db_and_client():
...
def test_first_user_can_be_created_anonymously_on_fresh_installation():
    db, client = create_test_db_and_client()
    try:
        seed_identity(db)
...
def test_second_user_requires_authentication_and_permission():
    db, client = create_test_db_and_client()
    try:
        seed_identity(db)
        school = create_sample_school(db)
...
def test_existing_authorization_behavior_unchanged_for_other_endpoints():
    db, client = create_test_db_and_client()
    try:
        seed_identity(db)
        school = create_sample_school(db)
